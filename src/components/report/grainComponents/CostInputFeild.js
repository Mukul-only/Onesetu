...
const CostInputFeild = ({ hash, id, onChange }) => {
  ...
  return (
    <div className="flex space-x-2 mt-8">
      <label
        htmlFor={id}
        className={`flex justify-start h-12 pl-2 pr-1 items-center min-w-[140px] md:min-w-[12rem] rounded-lg border-2 space-x-2 cursor-pointer ${
          highlight
      ...
      >
        ...
        <p className="text-xs md:text-base">{id}</p>
      </label>
      <FormInput
        input={{
          type: "number",
